...
def test_load_command_invalid_mode():
...
def test_load_command_full_mode_connection_error(mock_full_load):
...
@patch("py_load_chembl.cli.api.full_load")
def test_load_command_standard_representation(mock_full_load):
    ...
    assert result.exit_code == 0
    assert "Using 'standard' representation" in result.stdout
    mock_full_load.assert_called_once()
